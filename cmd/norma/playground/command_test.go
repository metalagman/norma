...
func TestRunCodexACPOneShot(t *testing.T) {
	...
	args := readArgsFile(t, argsFile)
	wantArgs := []string{"playground", "codex-acp-bridge", "--codex-arg", "--trace"}
	for _, want := range wantArgs {
		if !containsArg(args, want) {
			t.Fatalf("args %v do not contain %q", args, want)
		}
...
func TestBuildCodexACPBridgeCommand(t *testing.T) {
	got := buildCodexACPBridgeCommand(codexACPBridgeOptions{CodexArgs: []string{"--trace", "--foo=bar"}})
	want := []string{"codex", "mcp-server", "--trace", "--foo=bar"}
	if strings.Join(got, " ") != strings.Join(want, " ") {
		t.Fatalf("buildCodexACPBridgeCommand() = %v, want %v", got, want)
	}
}
...
func TestBuildCodexACPCommand(t *testing.T) {
	...
	want := []string{"/tmp/norma", "playground", "codex-acp-bridge", "--codex-arg", "--trace", "--codex-arg", "--raw"}
	if strings.Join(got, " ") != strings.Join(want, " ") {
...
func TestCodexACPBridgePromptUsesCodexThenReply(t *testing.T) {
	...
	agent := newCodexACPBridgeAgent(fakeSession)
...
func TestCodexACPBridgeCancelStopsPrompt(t *testing.T) {
...
func TestRunCodexACPBridgeForwardsCodexArgs(t *testing.T) {
	wrapper, argsFile := writeCodexMCPWrapper(t, "codex-tools")
	codexDir := t.TempDir()
	...
	var stderr bytes.Buffer
	runErr := runCodexACPBridge(
		context.Background(),
		t.TempDir(),
		codexACPBridgeOptions{CodexArgs: []string{"--trace"}},
		strings.NewReader(""),
		io.Discard,
		&stderr,
	)
	if runErr != nil {
		t.Fatalf("runCodexACPBridge() error = %v; stderr=%s", runErr, stderr.String())
	}
