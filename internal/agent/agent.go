...
func agentPrompt(req model.AgentRequest, modelName string) (string, error) {
	...
	b.WriteString("- Write only inside the provided step_dir (except Do may modify repo files to implement the selected issue).\n")
	...
	case "do":
		b.WriteString("Role guidance: execute only the locked slice; implement the selected issue in the repo and produce evidence under files/.\n")
