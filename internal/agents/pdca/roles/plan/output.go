...
func (strct *PlanWorkPlan) MarshalJSON() ([]byte, error) {
	...
	comma = true
    // Marshal the "next_task_id" field
    if comma {
        buf.WriteString(",")
    }
    buf.WriteString("\"next_task_id\": ")
	if tmp, err := json.Marshal(strct.NextTaskId); err != nil {
		return nil, err
 	} else {
 		buf.Write(tmp)
	}
...
func (strct *PlanWorkPlan) UnmarshalJSON(b []byte) error {
    ...
    for k, v := range jsonMap {
        ...
        case "next_task_id":
            if err := json.Unmarshal([]byte(v), &strct.NextTaskId); err != nil {
                return err
             }
        case "stop_triggers":
